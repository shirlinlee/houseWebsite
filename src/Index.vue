<template>
  <div id="wrapper">
      ...
      <section class="kv">
        ...
        <div class="scroll" @click="$store.commit('nav', 0);">scroll<span></span></div>
      </section>
      ...
      <section class="space">
        <transition name="fade">
          <main  v-for="(space,index) in spaces" :key="'s'+index" v-if="spaceShow === index">
            <div class="picBox" :class="'pic'+(index+1)"></div>
            <div class="des">
      ...
      <section class="info">
        <div class="content">
          <a class="map por" href="javascript:;" v-viewer="options">
            <span class="zoom abs"><img src="/static/asset/svg/zoom.svg"/></span>
            <img src="/static/img/map.png" class="float"/>
          </a>
...
export default {
  ...
  methods:{
    ...
    gotoPage(val) {
      var el = $('#wrapper>section').eq(val).offset().top;
      this.$body.animate({ scrollTop: el }, 800);
    },
    archiHandler(val) {
      var newVal = this.archiShow + val;
      if(newVal <= 0) newVal = 0;
      if(newVal >= this.archis.length ) newVal = this.archis.length-1;
      this.archiShow = newVal;
    },
    spaceHandler(val) {
      var newVal = this.spaceShow + val;
      if(newVal <= 0) newVal = 0;
      if(newVal >= this.spaces.length ) newVal = this.spaces.length-1;
    ...
    lifeHandler(val) {
      ...
      if(newVal >= this.lives.length ) newVal = this.lives.length-1;
